...
namespace MtC.Tools.Quadtree.Example.Step3Event
{
    ...
    public class QuadtreeEvent<T>
    {
        QuadtreeWithEventDelegateField _field;

        float _maxRadius = Mathf.NegativeInfinity;

        QuadtreeEvent<T> _root;
        QuadtreeEvent<T> _parent;
        QuadtreeEvent<T> _upperRightChild;
        QuadtreeEvent<T> _lowerRightChild;
        QuadtreeEvent<T> _lowerLeftChild;
        QuadtreeEvent<T> _upperLeftChild;

        List<QuadtreeLeafEvent<T>> _leafs = new List<QuadtreeLeafEvent<T>>();

        int _maxLeafsNumber;
        float _minSideLength;

        public QuadtreeEvent(float top, float right, float bottom, float left, int maxLeafNumber, float minSideLength, QuadtreeEvent<T> root = null, QuadtreeEvent<T> parent = null)
        {
        ...
        //存入
        public bool SetLeaf(QuadtreeLeafEvent<T> leaf)
        {
            if (DontHaveChildren())
        ...
        bool SetLeafToSelf(QuadtreeLeafEvent<T> leaf)
        {
        ...
        void UpdateMaxRadiusWhenSetLeaf(QuadtreeLeafEvent<T> leaf)
        {
        ...
        bool SetLeafToChildren(QuadtreeLeafEvent<T> leaf)
        {
        ...
        void Split()
        {
            ...
            _upperRightChild = new QuadtreeEvent<T>(_field.top, _field.right, yCenter, xCenter, _maxLeafsNumber, _minSideLength, _root, this);
            _lowerRightChild = new QuadtreeEvent<T>(yCenter, _field.right, _field.bottom, xCenter, _maxLeafsNumber, _minSideLength, _root, this);
            _lowerLeftChild = new QuadtreeEvent<T>(yCenter, xCenter, _field.bottom, _field.left, _maxLeafsNumber, _minSideLength, _root, this);
            _upperLeftChild = new QuadtreeEvent<T>(_field.top, xCenter, yCenter, _field.left, _maxLeafsNumber, _minSideLength, _root, this);

            foreach (QuadtreeLeafEvent<T> leaf in _leafs)
        ...
        {
            List<QuadtreeLeafEvent<T>> resetLeafs = new List<QuadtreeLeafEvent<T>>();

            foreach (QuadtreeLeafEvent<T> leaf in _leafs)
                if (!_field.Contains(leaf.position))
                    resetLeafs.Add(leaf);

            foreach (QuadtreeLeafEvent<T> leaf in resetLeafs)
                ResetLeaf(leaf);
        }

        void ResetLeaf(QuadtreeLeafEvent<T> leaf)
        {
         ...
         */
        public T[] CheckCollision(QuadtreeLeafEvent<T> leaf)
        {
        ...
        T[] GetCollisionObjectsFromAChild(Vector2 checkPoint, float checkRadius, QuadtreeEvent<T> child)
        {
        ...
        //移除
        public bool RemoveLeaf(QuadtreeLeafEvent<T> leaf)
        {
            if (DontHaveChildren())
        ...
        bool RemoveLeafFromSelf(QuadtreeLeafEvent<T> leaf)
        {
        ...
        bool DoRemoveLeafFromSelf(QuadtreeLeafEvent<T> leaf)
        {
        ...
        bool RemoveLeafFromChildren(QuadtreeLeafEvent<T> leaf)
        {
        ...
        bool RemoveLeafInTotalTree(QuadtreeLeafEvent<T> leaf)
        {
            if (DontHaveChildren())
        ...
        bool RemoveLeafInTotalTreeFromChildren(QuadtreeLeafEvent<T> leaf)
        {
