...
{
    public class QuadtreeLeafAction<T>
    {
        public T obj
        {
            get { return _obj; }
        ...
        public QuadtreeLeafAction(T obj, Vector2 position, float radius)
        {
            _obj = obj;
            _position = position;
            _radius = radius;
    ...
    public class QuadtreeAction<T>
    {
        QuadtreeFieldAction _field;

        float _maxRadius = Mathf.NegativeInfinity;

        QuadtreeAction<T> _root;
        QuadtreeAction<T> _parent;
        QuadtreeAction<T> _upperRightChild;
        QuadtreeAction<T> _lowerRightChild;
        QuadtreeAction<T> _lowerLeftChild;
        QuadtreeAction<T> _upperLeftChild;

        List<QuadtreeLeafAction<T>> _leafs = new List<QuadtreeLeafAction<T>>();
        ...
        //存入
        public bool SetLeaf(QuadtreeLeafAction<T> leaf)
        {
            if (DontHaveChildren())
        ...
        void UpdateMaxRadiusWhenSetLeaf(QuadtreeLeafAction<T> leaf)
        {
        ...
        {
            List<QuadtreeLeafAction<T>> resetLeafs = new List<QuadtreeLeafAction<T>>();

            foreach (QuadtreeLeafAction<T> leaf in _leafs)
                if (!_field.Contains(leaf.position))
                    resetLeafs.Add(leaf);

            foreach (QuadtreeLeafAction<T> leaf in resetLeafs)
                ResetLeaf(leaf);
        }
        ...
        //检测
        public T[] CheckCollision(QuadtreeLeafAction<T> leaf)
        {
        ...
        bool RemoveLeafFromSelf(QuadtreeLeafAction<T> leaf)
        {
        ...
        bool DoRemoveLeafFromSelf(QuadtreeLeafAction<T> leaf)
        {
    ...
    public class QuadtreeFieldAction
    {
        public float top
        {
            get { return _top; }
        ...
        public QuadtreeFieldAction(float top, float right, float bottom, float left)
        {
            _top = top;
            _right = right;
            _bottom = bottom;
