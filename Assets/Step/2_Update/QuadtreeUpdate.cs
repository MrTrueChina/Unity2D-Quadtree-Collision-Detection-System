...
namespace MtC.Tools.Quadtree.Example.Step2Update
{
    ...
    public class QuadtreeUpdate<T>
    {
        QuadtreeWithUpdateField _field;

        float _maxRadius = Mathf.NegativeInfinity;

        QuadtreeUpdate<T> _root;
        /*
         *  根节点，前面已经说到更新位置时需要从根节点再次存入叶子。
         *  虽然可以通过向上递归父节点查到根节点但找个字段存下来计算量小。
         */

        QuadtreeUpdate<T> _parent;
        QuadtreeUpdate<T> _upperRightChild;
        QuadtreeUpdate<T> _lowerRightChild;
        QuadtreeUpdate<T> _lowerLeftChild;
        QuadtreeUpdate<T> _upperLeftChild;

        List<QuadtreeLeafUpdate<T>> _leafs = new List<QuadtreeLeafUpdate<T>>();

        int _maxLeafsNumber;
        float _minSideLength;

        //构造，构造方法增加根节点参数，如果不传参则设为自身（只有创建根节点时不传参）
        public QuadtreeUpdate(float top, float right, float bottom, float left, int maxLeafNumber, float minSideLength, QuadtreeUpdate<T> root = null, QuadtreeUpdate<T> parent = null)
        {
         ...
         */
        public bool SetLeaf(QuadtreeLeafUpdate<T> leaf)
        {
            if (DontHaveChildren())
        ...
        bool SetLeafToSelf(QuadtreeLeafUpdate<T> leaf)
        {
        ...
        void UpdateMaxRadiusWhenSetLeaf(QuadtreeLeafUpdate<T> leaf)
        {
        ...
        bool SetLeafToChildren(QuadtreeLeafUpdate<T> leaf)
        {
        ...
        void Split()
        {
            ...
            _upperRightChild = new QuadtreeUpdate<T>(_field.top, _field.right, yCenter, xCenter, _maxLeafsNumber, _minSideLength, _root, this);
            _lowerRightChild = new QuadtreeUpdate<T>(yCenter, _field.right, _field.bottom, xCenter, _maxLeafsNumber, _minSideLength, _root, this);
            _lowerLeftChild = new QuadtreeUpdate<T>(yCenter, xCenter, _field.bottom, _field.left, _maxLeafsNumber, _minSideLength, _root, this);
            _upperLeftChild = new QuadtreeUpdate<T>(_field.top, xCenter, yCenter, _field.left, _maxLeafsNumber, _minSideLength, _root, this);

            foreach (QuadtreeLeafUpdate<T> leaf in _leafs)
        ...
        {
            List<QuadtreeLeafUpdate<T>> resetLeafs = new List<QuadtreeLeafUpdate<T>>();
            /*
            ...
            foreach (QuadtreeLeafUpdate<T> leaf in resetLeafs)
                ResetLeaf(leaf);
        }

        void ResetLeaf(QuadtreeLeafUpdate<T> leaf)
        {
        ...
        T[] GetCollisionObjectsFromAChild(Vector2 checkPoint, float checkRadius, QuadtreeUpdate<T> child)
        {
        ...
        //移除，增加了全树移除
        public bool RemoveLeaf(QuadtreeLeafUpdate<T> leaf)
        {
            if (DontHaveChildren())
        ...
        bool RemoveLeafFromSelf(QuadtreeLeafUpdate<T> leaf)
        {
        ...
        bool DoRemoveLeafFromSelf(QuadtreeLeafUpdate<T> leaf)
        {
         ...
         */
        bool RemoveLeafInTotalTree(QuadtreeLeafUpdate<T> leaf)
        {
            if (DontHaveChildren())
        ...
        bool RemoveLeafInTotalTreeFromChildren(QuadtreeLeafUpdate<T> leaf)
        {
